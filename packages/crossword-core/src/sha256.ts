/**
 * Small platform-neutral SHA-256 implementation.
 *
 * Save and migration checksums must be identical in Browser, AppsInToss,
 * Android and iOS. Depending on `crypto.subtle` would make the algorithm vary
 * in older WebViews, while importing a native/Node crypto SDK would violate the
...
export function sha256Hex(value: string): string {
  const source = encodeUtf8Bytes(value);
...
export function sha256Checksum(value: string): string {
