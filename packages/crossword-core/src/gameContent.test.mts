    ...
    entries: [
      {
        ...
        answerCells: ["가", "나"],
        clue: "fixture clue",
        ...fixtureAttribution("fixture-a1", "fixture clue"),
        clueSource: "manual",
        direction: "across",
        row: 0,
        col: 0,
      ...
      },
      {
        ...
        answerCells: ["가", "다"],
        clue: "fixture clue 2",
        ...fixtureAttribution("fixture-d1", "fixture clue 2"),
        clueSource: "manual",
        direction: "down",
        row: 0,
        col: 0,
...
function validate(value: unknown, requestedContentLocale = "ko-KR") {
  return validateGameContentV1(value, {
    ...
    verifyChecksum: (content) => content.contentChecksum === "checksum-fixture",
  });
}
...
describe("game-content/1 fail-closed validator", () => {
  ...
  test("entry 출처·라이선스·분야 메타데이터 누락을 허용하지 않는다", () => {
    const fixture = createFixture() as unknown as {
      entries: Array<Record<string, unknown>>;
    };
    delete fixture.entries[0].sourceEntryId;
    delete fixture.entries[1].domainTags;
    const result = validate(fixture);

    assert.equal(result.pass, false);
    assert.ok(
      result.issues.some((issue) => issue.path === "entries[0].sourceEntryId"),
    );
    assert.ok(
      result.issues.some((issue) => issue.path === "entries[1].domainTags"),
    );
  });

  test("schema 밖의 top-level·entry 필드를 허용하지 않는다", () => {
    const fixture = createFixture() as GameContentV1 & {
      unexpectedTopLevel?: string;
    };
    const firstEntry = fixture
      .entries[0] as GameContentV1["entries"][number] & {
      unexpectedEntryField?: string;
    };
    fixture.unexpectedTopLevel = "must fail";
    firstEntry.unexpectedEntryField = "must fail";

    const result = validate(fixture);
    assert.equal(result.pass, false);
    assert.ok(
      result.issues.some((issue) => issue.path === "unexpectedTopLevel"),
    );
    assert.ok(
      result.issues.some(
        (issue) => issue.path === "entries[0].unexpectedEntryField",
      ),
    );
  });

  test("canonical SHA-256은 payload 변경을 감지한다", () => {
    const fixture = createFixture();
    fixture.contentChecksum = calculateGameContentChecksum(fixture);
    assert.equal(verifyGameContentChecksum(fixture), true);

    fixture.entries[0].clue = "changed fixture clue";
    assert.equal(verifyGameContentChecksum(fixture), false);
  });
