...
describe("launchConfig: 막힘 힌트/피드백 튜닝값(#172)", () => {
  ...
  it("미설정(빈 값) 시 현재 값으로 폴백한다", () => {
    ...
    assert.equal(config.stuckHintFirstInputIdleMs, 6000);
  ...
  it("Remote Config 기본값 맵에 신규 키가 등록된다", () => {
    ...
    assert.equal(defaults[launchConfigKeys.stuckHintFirstInputIdleMs], 6000);
    assert.equal(
  ...
  it("허용 범위를 벗어나면 clamp된다", () => {
    ...
    const tooHigh = normalizeLaunchConfig({
      ...
      stuckHintFirstInputIdleMs: 999999,
    ...
    assert.equal(tooHigh.stuckHintFirstInputIdleMs, 120000);
  ...
  it("NaN 등 비유한 값은 기본값으로 폴백한다", () => {
    const config = normalizeLaunchConfig({
      ...
      stuckHintFirstInputIdleMs: Number.NaN,
