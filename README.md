...
| `--keep=14`          |                                       `14` | append 시 유지할 최근 퍼즐 수           |
...
| `--maxSyllableAnswers=3`       |    `3` | 한 판에서 같은 음절을 쓰는 정답 수 상한 |
| `--diversityHistory=7`         |    `7` | 비교할 최근 동일 난이도 퍼즐 수         |

`generation-report.json`에는 날짜별 retry, 후보별 실패 사유, 최종 채택된 판의 품질 게이트 결과가 기록됩니다.

운영 배치는 매일 한 번 Easy 5×5, Hard 8×8 두 판을 만들고 `--append --keep=14`로 최근 7일치를 유지합니다. 각 퍼즐은 `packId`, `puzzleId`, `slotId`, `publishedAt`를 가지며, 앱의 로컬 진행 상태는 `puzzleId` 기준으로 저장됩니다.

같은 날짜에 먼저 생성된 난이도의 정답과 같은 난이도의 최근 7판 정답은 다음 슬롯 후보에서 제외합니다. 이때 정답 문자열이 같은 단어뿐 아니라 어근(2음절 연속 조각)을 공유하는 단어까지 함께 빼므로, 어제 `대학생`을 썼으면 오늘 `학생`도 후보에 오르지 않습니다. `--maxAnswerReuse`는 날짜가 다른 최근 동일 난이도 퍼즐과의 재사용 비율을 제한하는 별도 기준입니다.

난이도는 쉬움/어려움 두 단계이며, 어휘가 아니라 **보드 크기와 배치 단어 수**로만 가릅니다. 두 티어는 같은 워드뱅크 전체를 후보로 쓰므로 어휘 엔트로피가 최대가 되고, 완료 부담이 easy 9~10단어 대 hard 18~22단어로 벌어져 체감 난도를 만듭니다.

한 판 안에서도 어근을 공유하는 정답은 쓰지 않습니다. 크로스워드 생성기는 교차가 쉬운 단어를 선호하기 때문에 제약이 없으면 `대학생 / 여학생 / 유학생 / 학생 / 대학`처럼 한 어근에서 파생된 단어 묶음으로 수렴합니다. 배치 단계에서 2음절 이상 겹치는 정답 조합을 잘라내고, 같은 음절을 쓰는 정답이 `--maxSyllableAnswers`개를 넘으면 후보에서 제외합니다. 1음절 공유는 교차 그 자체이므로 허용합니다.
...
3. 확인된 코드를 GitHub 저장소의 **repository variable `RETURN_REMINDER_TEMPLATE_CODE`**(Settings > Secrets and variables > Actions > Variables)로 등록합니다. `Deploy AIT` 워크플로우가 이 값을 `VITE_RETURN_REMINDER_TEMPLATE_CODE`로 주입합니다. 값이 없거나 공백이면 워크플로가 의존성 설치·빌드 전에 실패합니다.
4. 배포 후 GA4 `return_reminder_result`의 `template_code_source`(`env`/`default`), `outcome`, `error_code`, `error_wrapper_code`, `stage` 분포를 모니터링해 `template_code_source=env`와 최초의 `agreed`/`rejected`가 관측되는지 확인합니다.
