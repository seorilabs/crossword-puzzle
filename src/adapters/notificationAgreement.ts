...
export function requestReturnReminderAgreement(): Promise<ReturnReminderOutcome> {
  return new Promise((resolve) => {
    let settled = false;
    let cleanup: (() => void) | undefined;
    let timer: ReturnType<typeof setTimeout> | undefined = undefined;

    const finish = (outcome: ReturnReminderOutcome) => {
      if (settled) {
        return;
      }
      settled = true;
    ...
    timer = setTimeout(() => finish("error"), AGREEMENT_TIMEOUT_MS);
  });
}
