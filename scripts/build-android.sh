...
for variable_name in \
  RELEASE_TAG ANDROID_VERSION_NAME ANDROID_VERSION_CODE \
...
github_token_file="$credential_dir/github-packages-token"
for path in \
  "$firebase_base64" "$keystore_base64" "$keystore_password_file" \
  "$key_password_file" "$github_token_file"; do
  [ -s "$path" ] || {
...
version_output="$(node scripts/resolve-release-version.mjs --tag "$RELEASE_TAG")"
case "$version_output" in
  *"Resolved release version: $ANDROID_VERSION_NAME ($RELEASE_TAG), Android versionCode $ANDROID_VERSION_CODE"*) ;;
  *)
    echo "릴리즈 태그와 Android 버전 입력이 일치하지 않습니다." >&2
    exit 1
    ;;
esac
...
{
  printf '@seorilabs:registry=https://npm.pkg.github.com\n'
  printf '//npm.pkg.github.com/:_authToken=%s\n' "$github_token"
} > "$npmrc_file"
chmod 0600 "$npmrc_file"
unset github_token

NPM_CONFIG_USERCONFIG="$npmrc_file" npm ci
NPM_CONFIG_USERCONFIG="$npmrc_file" npm ci --prefix apps/mobile
...
echo "Signed AAB 생성 완료: path=$AAB_PATH bytes=$(wc -c < "$output_aab" | tr -d ' ') release=$RELEASE_TAG versionCode=$ANDROID_VERSION_CODE"
